...
- `calculate_*(a: number, b: number): number` — math operations via `math_*.ts` modules (includes `calculate_fwakg`, `calculate_udtsj`)
- `multiply_udtsj(a: number, b: number): number` — multiplication in `math_udtsj.ts`
