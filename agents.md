...
- `calculate_*(a: number, b: number): number` — math operations via `math_*.ts` modules (includes `calculate_fwakg`, `calculate_rz23k`)
- `subtract_rz23k(a: number, b: number): number` — subtraction via `math_rz23k.ts`
