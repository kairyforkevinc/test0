...
- `calculate_*(a: number, b: number): number` — math operations via `math_*.ts` modules (includes `calculate_fwakg`, `calculate_v565v`)
- `multiply_v565v(a: number, b: number): number` — multiplication in `math_v565v.ts`
