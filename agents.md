...
- `add_*(a: number, b: number): number` — addition (multiple variants, includes `add_fyizk`, `add_wt6vj`, `add_nuogg`, `add_p5ru9_a`)
- `subtract_*(a: number, b: number): number` — subtraction (multiple variants, includes `subtract_fyizk`, `subtract_bwv2l_c`, `subtract_wt6vj`, `subtract_nuogg`, `subtract_p5ru9_c`)
- `multiply_*(a: number, b: number): number` — multiplication (multiple variants, includes `multiply_fyizk`, `multiply_bwv2l_b`, `multiply_wt6vj`, `multiply_nuogg`, `multiply_p5ru9_b`, `multiply_sb1jq_b`)
- `divide_*(a: number, b: number): number` — division with zero-check (multiple variants, includes `divide_fyizk`, `divide_wt6vj`, `divide_nuogg`)
