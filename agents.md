...
- `calculate_*(a: number, b: number): number` — math operations via `math_*.ts` modules (includes `calculate_fwakg`, `calculate_v2kbe`)
- `multiply_v2kbe(a: number, b: number): number` — multiplication via `math_v2kbe.ts`
