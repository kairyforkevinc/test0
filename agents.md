...
- `greet_mmgvbzws(name: string): string` — returns `"Hello, {name}!"`, throws `Error('Name cannot be empty')` for empty/whitespace input (in `src/greet_mmgvbzws.ts`)
- `greet_mmgxhaa6(name: string): string` — returns `"Hello, {name}!"`, throws `Error('Name cannot be empty')` for empty/whitespace input (in `src/greet_mmgxhaa6.ts`)
...
- `calculate_*(a: number, b: number): number` — math operations via `math_*.ts` modules (includes `calculate_fwakg`, `calculate_vtquj`, `calculate_q0aq8`, `calculate_yjy7a`)
