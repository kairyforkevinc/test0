...
- `subtract_*(a: number, b: number): number` — subtraction (multiple variants, includes `subtract_fyizk`, `subtract_bwv2l_c`)
- `multiply_*(a: number, b: number): number` — multiplication (multiple variants, includes `multiply_fyizk`, `multiply_bwv2l_b`)
- `divide_*(a: number, b: number): number` — division with zero-check (multiple variants, includes `divide_fyizk`)
- `calculate_*(a: number, b: number): number` — math operations via `math_*.ts` modules (includes `calculate_fwakg`, `calculate_vtquj`)
