...
- `multiply_*(a: number, b: number): number` — multiplication (multiple variants, includes `multiply_fyizk`, `multiply_bwv2l_b`, `multiply_wt6vj`, `multiply_kz1ec`)
- `divide_*(a: number, b: number): number` — division with zero-check (multiple variants, includes `divide_fyizk`, `divide_wt6vj`)
- `calculate_*(a: number, b: number): number` — math operations via `math_*.ts` modules (includes `calculate_fwakg`, `calculate_vtquj`, `calculate_kz1ec`)
